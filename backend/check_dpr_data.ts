...
const checkDPRData = async () => {
    try {
        ...
        const materials = await sequelize.query(`SELECT id, name, material_code FROM materials`, { type: 'SELECT' })
        console.log('\n--- MATERIALS ---')
        ...
        const itemConsumption = await sequelize.query(`
            SELECT sti.id, sti.transaction_id, m.name as material_name, sti.quantity, sti.unit
