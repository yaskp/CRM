...
file_path = r'd:\CRM\backend\src\utils\pdfGenerator.ts'
new_code = r"""export const generatePurchaseOrderPDF = (po: any, stream: any) => {
    ...
    const companyGSTIN = 'GSTIN: 24AAAFV7277F1Z5' // Updated
...
start_marker = "export const generatePurchaseOrderPDF = (po: any, stream: any) => {"
start_idx = content.find(start_marker)

if start_idx != -1:
    # We found the function. Since it's at the end of the file, we can just truncate and append.
    # But wait, we need to make sure we keep the imports and other functions logic if any.
    # The file has imports and `generateWorkOrderPDF` and `generateQuotationPDF`.
    # Based on view_file, `generatePurchaseOrderPDF` is the LAST function.
    
    # So we can just take everything up to `start_idx` and append new_code
    ...
    print("Function not found, appending.")
