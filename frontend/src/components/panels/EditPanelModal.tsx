...
import { Modal, Form, Row, Col, Input, Select, Divider, Typography, Space, Button } from 'antd'
...
const EditPanelModal = ({ open, onCancel, onSubmit, loading, editingPanel }: EditPanelModalProps) => {
    ...
    useEffect(() => {
        if (open && editingPanel) {

            form.setFieldsValue({
                ...editingPanel,
                length: editingPanel.length,
                width: editingPanel.width,
                depth: editingPanel.design_depth,
                top_rl: editingPanel.top_rl,
                bottom_rl: editingPanel.bottom_rl,
                reinforcement_ton: editingPanel.reinforcement_ton,
                no_of_anchors: editingPanel.no_of_anchors,
                anchor_length: editingPanel.anchor_length,
                anchor_capacity: editingPanel.anchor_capacity,
                concrete_design_qty: editingPanel.concrete_design_qty,
                grabbing_qty: editingPanel.grabbing_qty,
                stop_end_area: editingPanel.stop_end_area,
                guide_wall_rm: editingPanel.guide_wall_rm,
                ramming_qty: editingPanel.ramming_qty,
                no_of_layers: editingPanel.anchors?.length || 0,
                anchors: editingPanel.anchors || []
            })
        }
    }, [open, editingPanel, form])
    ...
    const hasDPR = editingPanel?.dprRecords?.length > 0 || editingPanel?.consumptions?.length > 0
        ...
        <Modal
            title={
                <Space>
                    ...
                    <span>Edit Panel Dimensions — {editingPanel?.panel_identifier}</span>
                </Space>
            }
            open={open}
            onOk={() => form.submit()}
            ...
            okText="Save Changes"
            okButtonProps={{ danger: !!hasDPR }}
        >
            {hasDPR && (
                <div style={{ background: '#fef3c7', border: '1px solid #f59e0b', borderRadius: 8, padding: '10px 14px', marginBottom: 16, display: 'flex', gap: 10 }}>
                    <ExclamationCircleOutlined style={{ color: '#d97706', marginTop: 2, flexShrink: 0 }} />
                    <div>
                        <Text strong style={{ color: '#92400e' }}>DPR records exist for this panel.</Text><br />
                        <Text style={{ fontSize: 12, color: '#78350f' }}>
                            Changing L, W, or D will automatically recalculate concrete volume, grabbing & guide wall quantities.
                            DPR progress percentages based on design depth will also update.
                        </Text>
                    </div>
                </div>
            )}
            ...
            <Form form={form} layout="vertical" onFinish={onSubmit} onValuesChange={onFormValuesChange}>
                ...
                <Form.List name="anchors" initialValue={editingPanel?.anchors && editingPanel.anchors.length > 0 ? editingPanel.anchors : []}>
                    {(fields, { add, remove }) => (
                        <>
